# 🎥 Demo Video

## 📹 Project Demonstration

Watch the complete working demonstration of **FraudShieldAI** including:

- 🔐 Secure Login System
- 📊 Real-Time Fraud Dashboard
- 💳 Transaction Monitoring
- 📈 Analytics & Visualization
- 🚨 Alert Management
- 🤖 AI Insights
- 💬 AI Chat Assistant
- 📤 CSV Export Functionality

---

## 🎬 Demo Video

[▶️ Watch Demo Video](demo/FraudShieldAI_Demo.mov)

---

## 📌 Features Demonstrated

✔️ Fraud Detection using Machine Learning  
✔️ XGBoost Model Integration  
✔️ Real-Time Monitoring Dashboard  
✔️ Interactive Charts using Recharts  
✔️ AI-Powered Fraud Insights  
✔️ Alert Notification System  
✔️ Responsive Dark UI Design  
✔️ CSV Export Support  
✔️ Transaction Risk Analysis  

---
